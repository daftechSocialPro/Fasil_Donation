...
export default function BranchCreate({ user,setIsLodding }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      axios
        .post(urlBranch, formData)
        .then((res) => {
          setIsLodding(false)
          navigate('/branch')
          customToast('Branch Successfully created', 0)
        })
        .catch((err) => {
          setIsLodding(false)
          alert(err)
      ...
      <CCol xs={12}>
        <CCard className="mb-4">
          <CCardHeader style={{ backgroundColor: '#b2322a', color: '#fff' }} className="">
            <CRow>
              <CCol sm={10}>
                <strong>Branch</strong> <small>Create </small>
              </CCol>
            </CRow>
          </CCardHeader>
