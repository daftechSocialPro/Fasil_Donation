...
import { urlPartner } from '../../../endpoints'

export default function PartnerCreate({ user,setIsLodding }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      axios
        .post(urlPartner, formData)
        .then((res) => {
          setIsLodding(false)
          navigate('/partner')
          customToast('partner Successfully created', 0)
        })
        .catch((err) => {
          setIsLodding(false)
          alert(err)
      ...
      <CCol xs={12}>
        <CCard className="mb-4">
          <CCardHeader style={{ backgroundColor: '#b2322a', color: '#fff' }} className="">
            <CRow>
              <CCol sm={10}>
                <strong>Partner</strong> <small>Create </small>
              </CCol>
            </CRow>
          </CCardHeader>
