...
import { urlDonation } from '../../endpoints'

export default function DonationCreate({ user,setIsLodding }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      axios
        .post(urlDonation, formData)
        .then((res) => {
          setIsLodding(false)
          navigate('/donation')
          customToast('Donation Successfully created', 0)
        })
        .catch((err) => {
          setIsLodding(false)
          alert(err)
      ...
      <CCol xs={12}>
        <CCard className="mb-4">
          <CCardHeader style={{ backgroundColor: '#b2322a', color: '#fff' }} className="">
            <CRow>
              <CCol sm={10}>
                <strong>Donation</strong> <small>Create</small>
              </CCol>
            </CRow>
          </CCardHeader>
